...
@section('content')
    ...
    @endif
    @if (isset($chairman) || Auth::user()->hasAnyRole(['Super Admin', 'Admin']))
                                ...
                                <select class="form-select" aria-label="Default select example" id="teacher_id" name="teacher_id" required>
                                    <option selected>Select a Teacher</option>
                                        @foreach ($teachers as $teacher)
                                                <option value="{{ $teacher->id }}">{{ $teacher->name }} ({{ $teacher->teacher->designation }})</option>
                                        @endforeach
                                </select>
                                <br>
            ...
            <table class="table table-borderless">
                <thead>
                    <tr>
                        ...
                        @if($sections != [] && $sections[0]->teacher_id != Auth::id())
                ...
                <tbody>
                    <form action="{{ route('requestEnrollment') }}" method="POST">
                        @csrf
                        ...
                        <tr>
                            ...
                            @endif
                            <td>{{ $section->starting_time }} - {{ $section->ending_time }}</td>
                            @if ($section->room_no == null)
                                <td>TBA</td>
                            @else
                                <td>{{ $section->room_no }}</td>
                            @endif
                            <td>{{ $section->total_seats }}</td>
                            @if ($section->seats_available == null)
                                <td> - </td>
                            @else
                                <td>{{ $section->seats_available }}</td>
                            @endif
                            @if($sections[0]->teacher_id != Auth::id())
                            <td>
                                <button type="submit" class="btn btn-outline-danger" name='section_id' value="{{ $section->id }}">Request </button>
                            </td>
                            @endif
                        </tr>
                        @endforeach
