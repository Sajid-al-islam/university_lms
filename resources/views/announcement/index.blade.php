...
<div class="container-fluid h-100">
    ...
    <ul class="nav nav-tabs mb-3">
      <li class="nav-item">
        <a class="nav-link {{ (request()->route()->uri == 'dashboard/getAnnounement/{announcement}') ? 'active' : '' }}" aria-current="page" href="{{ route('showAnnouncement', $classModule->id) }}">Announcements</a>
      </li>
      <li class="nav-item">
        <a class="nav-link {{ request()->route()->uri == 'dashboard/getDeletedAnnouncements/{announcement}' ? 'active' : '' }}" aria-current="page" href="{{ route('deletedAnnouncements', $classModule->id) }}">Deleted Announcements</a>
      </li>
    </ul>
    ...
    @if($classModule->section->teacher_id == Auth::id())
      <button type='button' data-toggle='modal' data-target='#exampleModalCenter' class="btn btn-danger">Add Announcement</button>
              ...
              @forelse ($announcements as $announcement)
                ...
                @method('DELETE')
                    <li class="list-group-item d-flex justify-content-between">
                        <div class="my-3">
                          <p class=""><a href="{{ route('announcement.show', $announcement->id) }}" class="text-decoration-none">{{ $announcement->title }}</a></p>
                ...
                <h3 class="text-center">No Announcements Yet</h3>
                @endforelse
            </ul>
        @endif
    </div>
